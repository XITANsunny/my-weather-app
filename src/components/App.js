...
async function initApp(){
    ...
    if(navigator.geolocation){
        navigator.geolocation.getCurrentPosition(async({coords})=>{
            long = coords.longtitude;
            lat =coords.latitude;
            const weather = await fetchCurrentWeather(lat, long);
            console.log(weather);
        });
    }else{
        alert("Geolocation is not supported by this browser, try update the input box for the location");
    }
}
initApp();
//fetch the data
...
class App extends React.Component {
    ...
    searchCity = async city =>{
        await weather.get(`daily?city=${city}&key=28b9c5f6b18b44de8cc5fee76406cf72`).then(res=> {
        ...
        return (
            <div className="App">
                ...
                <div className="weatherContainer">   
                    ...
                    <div className="daily">
                    <WeatherBody day={'Monday'} icon={icon[0]} minTemp={minTemp[0]} maxTemp={maxTemp[0]} description={description[0]}/>
                    <WeatherBody day={'Tuesday'} icon={icon[1]} minTemp={minTemp[1]} maxTemp={maxTemp[1]} description={description[1]}/>
                    <WeatherBody day={'Wednesday'} icon={icon[2]} minTemp={minTemp[2]} maxTemp={maxTemp[2]} description={description[2]}/>
                    <WeatherBody day={'Thursday'} icon={icon[3]} minTemp={minTemp[3]} maxTemp={maxTemp[3]} description={description[3]}/>
                    <WeatherBody day={'Friday'} icon={icon[4]} minTemp={minTemp[4]} maxTemp={maxTemp[4]} description={description[4]}/>
                    </div>
                </div>
